...
setInterval(() => {
    ...
    for (let i in channels) {
        if (channels[i].members.array().length === 0) {
            channels[i].delete()
                .then(() => {
                    manager.nvyAddLog(`[LOG] -> Delete created channel`);
                })
                .catch(() => {
                    manager.nvyAddLog(`[ERROR LOG] -> [MEDIUM] -> Can't delete created channel`);
                });
        }
    }
}, 10000)
        ...
        guild.members.fetch(user.id).then(member => {
            ...
            guild.channels.create(name, options).then(data => {
                ...
                userVoiceState.setChannel(channelNew)
                    .then(res => {
                        manager.nvyAddLog(`[LOG] -> Create channel for ${user.id} with limit 2`);
                    })
                    .catch(err => {
                        manager.nvyAddLog(`[ERROR LOG] -> [LOW] -> ${user.id} fast leave from channel`);
                    });
    ...
    client.channels.fetch(voiceChannels.trio.id).then((channel: ds.VoiceChannel | null) => {
        ...
        guild.members.fetch(user.id).then(member => {
            ...
            guild.channels.create(name, options).then(data => {
                ...
                userVoiceState.setChannel(channelNew)
                    .then(res => {
                        manager.nvyAddLog(`[LOG] -> Create channel for ${user.id} with limit 3`);
                    })
                    .catch(err => {
                        manager.nvyAddLog(`[ERROR LOG] -> [LOW] -> ${user.id} fast leave from channel`);
                    });
    ...
    client.channels.fetch(voiceChannels.four.id).then((channel: ds.VoiceChannel | null) => {
        ...
        guild.members.fetch(user.id).then(member => {
            ...
            guild.channels.create(name, options).then(data => {
                ...
                userVoiceState.setChannel(channelNew)
                    .then(res => {
                        manager.nvyAddLog(`[LOG] -> Create channel for ${user.id} with limit 4`);
                    })
                    .catch(err => {
                        manager.nvyAddLog(`[ERROR LOG] -> [LOW] -> ${user.id} fast leave from channel`);
                    });
    ...
    client.channels.fetch(voiceChannels.five.id).then((channel: ds.VoiceChannel | null) => {
        ...
        guild.members.fetch(user.id).then(member => {
            ...
            guild.channels.create(name, options).then(data => {
                ...
                userVoiceState.setChannel(channelNew)
                    .then(res => {2
                        manager.nvyAddLog(`[LOG] -> Create channel for ${user.id} with limit 5`);
                    })
                    .catch(err => {
                        manager.nvyAddLog(`[ERROR LOG] -> [LOW] -> ${user.id} fast leave from channel`);
                    });
